...
/**
 * Preference - ReadWriteProperty class for Android SharedPreference
 */
open class Preference<T>(private val key: String, private val default: T): ReadWriteProperty<Any?, T> {
    /**
     * getValue - Overridden of ReadWriteProperty class
     */
    ...
    /**
     * setValue - Overridden method of ReadWriteProperty class
     */
    ...
    companion object {
        ...
        fun init(ctx: Context, name: String, mode: Int = Context.MODE_PRIVATE){
            prefSharedPreferences = ctx.getSharedPreferences(name, prefMode)
            prefName = name
            prefMode = mode
        }

        fun cleanPreferences(){
            prefSharedPreferences.edit().clear().apply()
        }
    }
}